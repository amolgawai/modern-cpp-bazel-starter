...
#
# 2018-03-31, Bendik Samseth - Relax debug output. - Keep a copy of the coverage output for later use. - Updated
# coverage exclude patterns.
#
# 2018-01-03, HenryRLee - Allow for *Clang compiler names, not just Clang.
#
# 2018-01-03, Bendik Samseth - Only check compiler compatibility if in a coverage build.
...
# Param _targetname     The name of new the custom make target Param _testrunner     The name of the target which runs
# the tests. MUST return ZERO always, even on errors. If not, no coverage report will be created! Param _outputname lcov
...
function(setup_target_for_coverage _targetname _testrunner _outputname)
    ...
    add_custom_target(
        ...
        COMMAND ${LCOV_PATH} --remove ${_outputname}.info '*/tests/*' '/usr/*' '*/external/*' '*/3rd/*' '*/_deps/*'
                '*/.cache/*' '/Applications/*' --output-file ${_outputname}.info.cleaned
    ...
    add_custom_command(
        ...
        COMMAND ;
        COMMENT
            "${BoldMagenta}Open ./${_outputname}/index.html in your browser to view the coverage report.${ColourReset}"
    )
...
if(cmake_build_type_tolower STREQUAL "coverage")
    ...
    # If unwanted files are included in the coverage reports, you can adjust the exclude patterns on line 83.
    # setup_target_for_coverage( coverage            # Name for custom target. ${TEST_MAIN}        # Name of the test
    # driver executable that runs the tests. # NOTE! This should always have a ZERO as exit code # otherwise the
    # coverage generation will not complete. coverage_out        # Name of output directory. )
else()
    add_custom_target(
        coverage
        ...
        COMMAND
            echo
            "${Green}Make a new build directory and rerun cmake with -DCMAKE_BUILD_TYPE=Coverage to enable this target.${ColorReset}"
    )
endif()
